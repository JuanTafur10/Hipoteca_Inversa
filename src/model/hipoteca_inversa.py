...
class ErrorFaltaTasaInteres(Exception):
    """
    Excepción lanzada cuando no se proporciona una tasa de interés válida.
    """
    def __init__(self, message="La tasa de interés es obligatoria y no puede ser nula o negativa."):
        super().__init__(message)

class ErrorFaltaMontoInicial(Exception):
    """
    Excepción lanzada cuando no se proporciona un monto inicial válido.
    """
    def __init__(self, message="El monto inicial es obligatorio y no puede ser nulo o negativo."):
        super().__init__(message)
...
class Hipoteca:
    ...
    def validar_datos(self):
        ...
        if self.tasa_de_interes_mensual is None or self.tasa_de_interes_mensual < 0:
            # La tasa de interés es obligatoria y debe ser mayor que 0.
            raise ErrorFaltaTasaInteres()
        
        if self.porcentaje_precio_real is None or self.porcentaje_precio_real <= 0:
            # El porcentaje del precio real es obligatorio y debe ser mayor que 0.
            raise ErrorFaltaMontoInicial()
